...
int main(void) {
    ...
    pa_sink_info **sink_info = get_available_sinks();
    if (!sink_info) {
    ...
    for (uint32_t i = 0; i < total_devices; i++) {
        ...
        int sample_rate = get_sample_rate(alsa_id, sink_info[i]);
        ...
        int min_channels = get_min_channels(alsa_id, sink_info[i]);
        int max_channels = get_max_channels(alsa_id, sink_info[i]);
