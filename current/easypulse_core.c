...
static void sink_cb(pa_context *c, const pa_sink_info *i, int eol, void *userdata) {
    ...
    if (manager->sink_count % INITIAL_ALLOCATION_SIZE == 0) {  // Resize every time we hit a multiple of INITIAL_ALLOCATION_SIZE
        size_t newSize = (manager->sink_count + INITIAL_ALLOCATION_SIZE) * sizeof(pulseaudio_sink);
        pulseaudio_sink *new_sinks = realloc(manager->sinks, newSize);
 ...
 */
bool load_sinks(pulseaudio_manager *self) {
    pa_operation *op;
...
/**
 * @brief Retrieve a list of available sinks.
 * @param self A pointer to the pulseaudio_manager instance.
 * @return A pointer to an array of available sinks. The caller is responsible for freeing this memory.
 */
pulseaudio_sink* get_sink_list(pulseaudio_manager *self) {
    if (!self || !self->sinks_loaded) {
        fprintf(stderr, "[ERROR]: Manager not initialized or sinks not loaded.\n");
        return NULL;
    }

    // Allocate memory for the list of sinks
    pulseaudio_sink *sinks_list = malloc(self->sink_count * sizeof(pulseaudio_sink));
    if (!sinks_list) {
        fprintf(stderr, "[ERROR]: Failed to allocate memory for sinks list.\n");
        return NULL;
    }

    // Copy sink information from the manager's sinks array
    for (uint32_t i = 0; i < self->sink_count; i++) {
        sinks_list[i] = self->sinks[i];
    }
...
/**
 * @brief Switches the sink (audio source) for the pulseaudio_manager.
 *
 * @param self Pointer to the pulseaudio_manager instance.
 * @param sink_index Index of the sink to switch to.
 * @return Boolean indicating success or failure.
 */
bool switch_sink(pulseaudio_manager *self, uint32_t sink_index) {
...
/**
 * @brief Retrieves the number of channels for a specified sink.
 *
 * @param sinks Pointer to an array of PulseSink structures.
 * @param sink_index Index of the sink whose number of channels is to be retrieved.
 *
 * @return Number of channels for the specified sink. Returns -1 on error.
 */
int get_sink_channels(const pulseaudio_sink *sinks, int sink_index) {
    if (!sinks || sink_index < 0) {
        fprintf(stderr, "[ERROR]: Invalid sinks array or sink index in get_sink_channels.\n");
        return -1;  // Return -1 or another indicator of failure
    }
    return sinks[sink_index].channel_map.channels;
}
...
bool set_volume(pulseaudio_manager *self, uint32_t sink_index, float percentage) {
    ...
    pa_operation *op2 = pa_context_get_sink_info_by_name(self->context, sink_name_to_change, set_volume_check_cb, self);
...
pulseaudio_manager *new_manager(void) {
    ...
    manager->sinks = (pulseaudio_sink *)malloc(5 * sizeof(pulseaudio_sink));
    if (!manager->sinks) {
        free(manager);
        return NULL;
    }
    ...
    manager->sinks_loaded = 0;
    manager->sinks = (pulseaudio_sink *)malloc(5 * sizeof(pulseaudio_sink));
    manager->sink_count = 0;
    manager->sinks_loaded = 0;  // Initialize to 0
    manager->load_sinks = load_sinks;
    manager->destroy = destroy;
    manager->switch_sink = switch_sink;
    manager->set_volume = set_volume;
    manager->get_active_sink = get_active_sink;
    manager->iterate = iterate;
    manager->get_sink_channels = get_sink_channels;
 ...
 * @param userdata User-provided data (expected to be a pointer to pulseaudio_manager).
 */
static void active_sink_cb(pa_context *c, const pa_server_info *info, void *userdata) {
    if (!info || !info->default_sink_name) {
        fprintf(stderr, "[ERROR]: Null pointer in active_sink_cb.\n");
        return;
    }
    (void) c;
    pulseaudio_manager *manager = (pulseaudio_manager *)userdata;
    ...
    for (uint32_t i = 0; i < manager->sink_count; i++) {
    ...
    //printf("[DEBUG, active_sink_cb()]: Active sink index is, %i\n", manager->active_sink_index);
    //printf("[DEBUG, active_sink_cb()]: Active sink name is, %s\n", manager->active_sink_name);
    pa_threaded_mainloop_signal(manager->mainloop, 0);
}
 ...
 * Once the server provides this information, the active_sink_cb callback is triggered to process the response.
 *
 * @param manager Pointer to the pulseaudio_manager instance.
 */
void get_active_sink(pulseaudio_manager *manager) {
    ...
    pa_context_get_server_info(manager->context, active_sink_cb, manager);
    ...
    // The original loop to wait for active_sink_name to be set
...
void destroy(pulseaudio_manager *self) {
    ...
    if (self) {
        for (uint32_t i = 0; i < self->sink_count; i++) {
            free(self->sinks[i].name);
            free(self->sinks[i].description);
        }
        free(self->sinks);
        free(self);
    }
