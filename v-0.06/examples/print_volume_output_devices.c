...
int main() {
    ...
    for (uint32_t i = 0; i < device_count; ++i) {
        ...
        const char *alsa_id = get_alsa_id(sinks[i]->name);
        printf("\tALSA ID: %s\n", alsa_id);
        ...
        for (uint8_t ch = 0; ch < sinks[i]->channel_map.channels; ++ch) {
            ...
            printf("\tChannel %u Volume: %.2f%%\n", ch, volume_percent);
        }
    }

    // Cleanup
    delete_output_devices(sinks);
    // You may need to add cleanup functions from system_query.c here to clean up PulseAudio
