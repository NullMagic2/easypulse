...
pulseaudio_manager *manager_create(void) {
    ...
    if (self->output_count > 0) {
        ...
        for (uint32_t i = 0; i < self->output_count; ++i) {
            ...
            //Initializing the stream like that is important so that we can modify the sample rate later.
            pa_sample_spec default_sample_spec = {
                .format = PA_SAMPLE_S16LE,
                .rate = self->outputs[i].sample_rate,
                .channels = (uint8_t) self->outputs[i].max_channels
            };

            self->outputs[i].stream = pa_stream_new(self->context, self->outputs[i].name, &default_sample_spec, NULL);
            if (!self->outputs[i].stream) {
                fprintf(stderr, "Failed to create stream for output device: %s\n", self->outputs[i].name);
                continue;
            }

            // Connect the stream with PA_STREAM_VARIABLE_RATE
            if (pa_stream_connect_playback(self->outputs[i].stream, NULL, NULL, PA_STREAM_VARIABLE_RATE, NULL, NULL) < 0) {
                fprintf(stderr, "Failed to connect playback stream for device: %s\n", self->outputs[i].name);
                pa_stream_unref(self->outputs[i].stream);
                self->outputs[i].stream = NULL;
                continue;
            }
    ...
    if (self->input_count > 0) {
        ...
        for (uint32_t i = 0; i < self->input_count; ++i) {
            ...
            pa_sample_spec default_sample_spec = {
                .format = PA_SAMPLE_S16LE,
                .rate = self->inputs[i].sample_rate,
                .channels = (uint8_t) self->inputs[i].max_channels
            };

            self->inputs[i].stream = pa_stream_new(self->context, self->inputs[i].name, &default_sample_spec, NULL);

            if (!self->inputs[i].stream) {
                fprintf(stderr, "Failed to create stream for output device: %s\n", self->inputs[i].name);
                continue;
            }

            // Connect the stream with PA_STREAM_VARIABLE_RATE to the specific device
            if (pa_stream_connect_playback(self->inputs[i].stream, self->inputs[i].code, NULL, PA_STREAM_VARIABLE_RATE, NULL, NULL) < 0) {
                fprintf(stderr, "Failed to connect playback stream for device: %s\n", self->inputs[i].code);
                pa_stream_unref(self->inputs[i].stream);
                self->inputs[i].stream = NULL;
                continue;
            }
...
uint32_t channel_index, bool mute_state) {
    ...
    if (!op) {
        fprintf(stderr, "Failed to start sink information operation.\n");
        return -1;
    }
    ...
    &(volume_data.new_volume), manager_set_output_channel_mute_state_cb2, NULL);
 ...
 *
 */
static void manager_set_input_mute_state_cb(pa_context *c, const pa_sink_info *info, int eol, void *userdata) {
    (void) c;
...
/**
 * @brief Sets the mute state for an entire PulseAudio input device.
 *
...
uint32_t channel_index, bool mute_state) {
    ...
    // Requesting information about the specified sink
    pa_operation *op = pa_context_get_sink_info_by_index(self->context, input_index, manager_set_input_mute_state_cb, self);
    ...
    op = pa_context_set_sink_volume_by_index(self->context, input_index,
    &(volume_data.new_volume), manager_set_input_mute_state_cb2, self);

    if (!op) {
        fprintf(stderr, "Failed to start sink volume set operation.\n");
        return -1;
    }
