 ...
 */
pa_sink_info **get_available_sinks(uint32_t card_index) {
...
/**
 * @brief Frees the dynamically allocated memory for the sink list.
 *
 * This function is used to free the memory allocated for the sink list
 * after it's no longer needed. This ensures that there are no memory leaks.
 *
 * @param data Pointer to the sink list data structure.
 */
static void free_sink_list(void) {
    if (shared_data_3.sinks) {
        free(shared_data_3.sinks);
    }
}
 ...
 */
int get_min_channels(const char *alsa_id) {
    snd_pcm_t *handle;
    snd_pcm_hw_params_t *params;
...
static void free_alsa_name(void) {
    if (shared_data_2.alsa_name) {
        free((char*)shared_data_2.alsa_name);
        shared_data_2.alsa_name = NULL;
    }
}
