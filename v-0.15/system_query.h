...
//Used by get_input_streams() to get a list of all Pulseaudio source outputs.
typedef struct input_stream_list {
    input_stream_info *outputs;
    uint32_t num_inputs;
} input_stream_list;
...
const pa_source_info *source_info);                                        //Gets the minimum output channels an ALSA card supports.
...
const pa_sink_info *sink_info);                                            //Gets the minimum output channels an ALSA card supports.
...
void input_streams_cleanup(input_stream_list *list);                       //Cleans up allocated input streams.
