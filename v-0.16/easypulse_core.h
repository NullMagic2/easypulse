...
uint32_t device_index);                                            //Changes the default output device.

bool manager_switch_default_input(pulseaudio_manager *self,
uint32_t device_index);                                            //Changes the default input device.

int manager_set_output_sample_rate(pulseaudio_manager *manager,
uint32_t device_index, int sample_rate);                           //Changes the output of an output device.

int manager_set_pulseaudio_global_rate(int sample_rate);           //Changes the output of an output device.

int manager_set_output_mute_state(pulseaudio_manager *self,
uint32_t output_index, uint32_t channel_index, bool mute_state);  //Changes a number of output channels to mute / unmuted.

int manager_set_input_mute_state(pulseaudio_manager *self,
uint32_t input_index, uint32_t channel_index, bool mute_state);   //Changes a number of input channels to mute / unmuted.

int manager_move_output_playback(pulseaudio_manager *manager,
uint32_t sink1_index, uint32_t sink2_index);                      //Moves playback from one sink to another.

bool manager_move_sink_input(pulseaudio_manager *manager,
uint32_t source_sink, uint32_t target_sink);                       //Moves sink input from a device to another
