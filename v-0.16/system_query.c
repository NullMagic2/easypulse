/**
 * @file system_query.c
 * @brief Functions for querying system audio properties using PulseAudio in a Linux environment.
 *
 * This implementation file provides a collection of functions designed to interact with the
 * PulseAudio sound server to query and manipulate various audio properties of a Linux system.
 * These functions allow for the examination and control of audio devices (sinks and sources),
 * such as enumerating available devices, retrieving their properties, checking and changing
 * volume and mute states, and managing audio profiles. The functionalities encapsulated in
 * this file are crucial for applications that need to interface with the system's audio
 * hardware and perform operations like audio routing, volume control, or retrieving hardware
 * information.
 *
 *
 * @author Mbyte2
...
// Structure to hold the sink profile information
typedef struct {
...
/**
 * @brief Frees memory allocated for an output_stream_list.
 *
 * This function is responsible for properly freeing the memory allocated for an
 * output_stream_list and its associated array of output_stream_info structures.
 * It frees the array of output_stream_info and then the output_stream_list structure itself.

 *
 * @param list A pointer to the output_stream_list to be freed. If this pointer is NULL,
 *             the function does nothing.
 */
void output_streams_cleanup(output_stream_list *list) {
    if (list != NULL) {
        // Iterate over each output_stream_info and free its dynamically allocated fields
        for (uint32_t i = 0; i < list->num_inputs; ++i) {
            free(list->inputs[i].name);
            free(list->inputs[i].driver);

            if (list->inputs[i].format != NULL) {
                pa_format_info_free(list->inputs[i].format);
            }

            if (list->inputs[i].proplist != NULL) {
                pa_proplist_free(list->inputs[i].proplist);
            }
        }

        // Free the array of output_stream_info
        free(list->inputs);

        // Free the output_stream_list structure itself
        free(list);
    }
}

/**
 * @brief Frees memory allocated for an input_stream_list.
 *
 * This function is responsible for properly freeing the memory allocated for an
 * input_stream_list and its associated array of input_stream_info structures.
 * It frees the array of input_stream_info and then the input_stream_list structure itself.
 *
 * Usage of this function is essential after you're done using an input_stream_list
 * to avoid memory leaks. This function safely handles NULL pointers, so it can be
 * called with input_stream_list structures that may not have been fully initialized.
 *
 * @param list A pointer to the input_stream_list to be freed. If this pointer is NULL,
 *             the function does nothing.
 */
void input_streams_cleanup(input_stream_list *list) {
    if (list != NULL) {
        // Iterate over each input_stream_info and free its dynamically allocated fields
        for (uint32_t i = 0; i < list->num_inputs; ++i) {
            free(list->outputs[i].name);
            free(list->outputs[i].driver);

            if (list->outputs[i].format != NULL) {
                pa_format_info_free(list->outputs[i].format);
            }

            if (list->outputs[i].proplist != NULL) {
                pa_proplist_free(list->outputs[i].proplist);
            }
        }

        // Free the array of input_stream_info
        free(list->outputs);

        // Free the input_stream_list structure itself
        free(list);
    }
}

/**
 * @brief Callback function for sink input information retrieval.
 *
 * This function is called by the PulseAudio context for each sink input
 * retrieved by pa_context_get_sink_input_info_list(). It allocates memory
 * for a new output_stream_info structure, copies the relevant sink input
 * information into it, and stores it in the output_stream_list.
 *
 * This function handles the allocation of memory for each output_stream_info
 * and ensures that relevant data from the pa_sink_input_info, such as the
 * index, parent index, and name, are copied over.
 *
 * @param c The PulseAudio context.
 * @param i The sink input information structure.
 * @param eol End of list indicator. If positive, it's the end of the list.
 * @param userdata User data pointer, cast to output_stream_list, passed to the function.
 */
static void get_output_streams_cb(pa_context *c, const pa_sink_input_info *i, int eol, void *userdata) {
    (void) c;
    ...
    output_stream_list *input_list = (output_stream_list *) userdata;

    // Resize the inputs array to accommodate one more output_stream_info
    input_list->inputs = realloc(input_list->inputs, (input_list->num_inputs + 1) * sizeof(output_stream_info));
    if (input_list->inputs == NULL) {
        fprintf(stderr, "Failed to allocate memory for sink input list\n");
        return;
    }

    output_stream_info *new_input = &input_list->inputs[input_list->num_inputs];
    new_input->index = i->index;
    new_input->owner_module = i->owner_module;
    new_input->parent_index = i->sink; // Index of the corresponding output device (sink)
    new_input->volume = i->volume;
    new_input->sample_spec = i->sample_spec;
    new_input->channel_map = i->channel_map;

    // Copy format info
    if (i->format) {
        new_input->format = pa_format_info_copy(i->format);
    } else {
        new_input->format = NULL;
    }

    // Copy proplist
    if (i->proplist) {
        new_input->proplist = pa_proplist_copy(i->proplist);
    } else {
        new_input->proplist = NULL;
    }

    // Copy name
    new_input->name = strdup(i->name);
    if (new_input->name == NULL) {
        fprintf(stderr, "Failed to allocate memory for sink input name\n");
    }

    // Copy driver
    new_input->driver = strdup(i->driver);
    if (new_input->driver == NULL) {
        fprintf(stderr, "Failed to allocate memory for sink input driver\n");
    }
...
/**
 * @brief Retrieves a list of current sink inputs from the PulseAudio server.
 *
 * This function initiates an asynchronous operation to retrieve the list
 * of sink inputs from the PulseAudio server. It allocates an instance of
 * output_stream_list and populates it with output_stream_info structures.
 * Each entry in the list is a dynamically allocated output_stream_info structure
 * containing the sink input details such as index, parent index, and name.
 *
 * The caller is responsible for freeing the memory allocated for each
 * output_stream_info structure and the output_stream_list itself after use.
 * The function blocks until the operation is completed or fails.
 * The sink input list will not be modified after the function returns.
 *
 * @param context A pointer to an initialized and connected pa_context object.
 * @return A pointer to an output_stream_list containing all retrieved sink inputs,
 *         or NULL if an error occurs.
 */
output_stream_list *get_output_streams(pa_context *context) {
    output_stream_list *input_list = malloc(sizeof(output_stream_list));
    if (!input_list) {
        fprintf(stderr, "Failed to allocate memory for output_stream_list\n");
        return NULL;
    }

    input_list->inputs = NULL;
    input_list->num_inputs = 0;

    // Initiate the operation to get the list of sink inputs
    pa_operation *op = pa_context_get_sink_input_info_list(context, get_output_streams_cb, input_list);
    if (!op) {
        fprintf(stderr, "Failed to start operation to get sink input list\n");
        free(input_list);
        return NULL;
    }
...
/**
 * @brief Callback function to process each input stream found.
 *
 * This function is called by the PulseAudio context for each source output.
 * It stores information about each input stream into an input_stream_list structure.
 * When the end of the list is reached, it signals the main loop to continue execution.
 *
 * @param c Pointer to the PulseAudio context (not used in this callback).
 * @param o Pointer to the pa_source_output_info structure containing the source output info.
 * @param eol End of list flag. If non-zero, indicates no more data is forthcoming.
 * @param userdata User data pointer, expected to be a pointer to an input_stream_list structure.
 */
static void get_input_streams_cb(pa_context *c, const pa_source_output_info *o, int eol, void *userdata) {
    (void) c;
    ...
    input_stream_list *input_list = (input_stream_list *) userdata;

    // Resize the outputs array to accommodate one more input_stream_info
    input_list->outputs = realloc(input_list->outputs, (input_list->num_inputs + 1) * sizeof(input_stream_info));
    if (input_list->outputs == NULL) {
        fprintf(stderr, "Failed to allocate memory for source output list\n");
        return;
    }

    // Reference the new element where we will store the info
    input_stream_info *new_info = &input_list->outputs[input_list->num_inputs];

    // Copy the relevant info from pa_source_output_info to input_stream_info
    new_info->index = o->index;
    new_info->owner_module = o->owner_module;
    new_info->parent_index = o->source; // Index of the corresponding input device (source)
    new_info->volume = o->volume;
    new_info->sample_spec = o->sample_spec;
    new_info->channel_map = o->channel_map;

    // Copy format info
    if (o->format) {
        new_info->format = pa_format_info_copy(o->format);
    } else {
        new_info->format = NULL;
    }

    // Copy proplist
    if (o->proplist) {
        new_info->proplist = pa_proplist_copy(o->proplist);
    } else {
        new_info->proplist = NULL;
    }

    // Copy name
    new_info->name = strdup(o->name);
    if (new_info->name == NULL) {
        fprintf(stderr, "Failed to allocate memory for source output name\n");
    }

    // Copy driver
    new_info->driver = strdup(o->driver);
    if (new_info->driver == NULL) {
        fprintf(stderr, "Failed to allocate memory for source output driver\n");
    }

    // Increment the inputs counter
    input_list->num_inputs++;
}

/**
 * @brief Retrieves a list of all input streams (source outputs) from the PulseAudio server.
 *
 * This function initiates an asynchronous operation to list all the source outputs (input streams)
 * currently available in the PulseAudio server. It waits for the operation to complete and
 * returns a list of input streams.
 *
 * @param context Pointer to the initialized and connected PulseAudio context.
 * @return Pointer to an input_stream_list structure containing the list of input streams.
 *         Returns NULL if the operation fails or memory allocation is not possible.
 */
input_stream_list *get_input_streams(pa_context *context) {
    input_stream_list *input_list = malloc(sizeof(input_stream_list));
    if (!input_list) {
        fprintf(stderr, "Failed to allocate memory for input_stream_list\n");
        return NULL;
    }

    input_list->outputs = NULL;
    input_list->num_inputs = 0;

    // Initiate the operation to get the list of source outputs
    pa_operation *op = pa_context_get_source_output_info_list(context, get_input_streams_cb, input_list);
    if (!op) {
        fprintf(stderr, "Failed to start operation to get source output list\n");
        free(input_list);
        return NULL;
    }
...
/**
 * @brief Callback function for retrieving the active profile information of a PulseAudio card.
 *
 * This function is called by PulseAudio in response to pa_context_get_card_info_by_name.
 * It processes the card information and stores the active profile if found.
 *
 * @param c Pointer to the PulseAudio context. Not used in this function.
 * @param i Pointer to the PulseAudio card information structure.
 * @param eol End-of-list flag. If positive, it indicates no more data is available.
 * @param userdata User-provided pointer to store the resulting profile information.
 *
 * @note The function assumes userdata is a pointer to card_profile_info structure.
 */
static void get_active_profile_cb(pa_context *c, const pa_card_info *i, int eol, void *userdata) {
    (void) c;

    fprintf(stderr, "[DEBUG, card_info_cb()], reached function\n");

    card_profile_info *info = (card_profile_info *)userdata;
    printf("[DEBUG] i is: %p\n", i);

    if (eol > 0 || !i) {
        pa_threaded_mainloop_signal(shared_data_1.mainloop, 0);
        return; // No more data, or null card info
    }

    if (i->active_profile && !info->found) {
        // Allocate memory for the new profile
        info->active_profile = malloc(sizeof(pa_card_profile_info));
        if (info->active_profile) {
            // Duplicate the name and description strings
            info->active_profile->name = strdup(i->active_profile->name);
            info->active_profile->description = strdup(i->active_profile->description);
            info->found = true;
        }
    }

    fprintf(stderr, "[DEBUG, card_info_cb()] card_info_cb is %s\n", i->active_profile->description);
}

/**
 * @brief Retrieves the active profile information for a specified PulseAudio card.
 *
 * This function initiates a query to PulseAudio to get information about a specific card.
 * It blocks until the callback (get_active_profile_cb) has processed the data.
 *
 * @param context Pointer to the PulseAudio context.
 * @param card_name Name of the PulseAudio card to query.
 *
 * @return Pointer to the active profile information on success, NULL on failure or if no active profile is found.
 *
 * @note The returned pointer should not be freed by the caller, as it points to statically allocated memory.
 */
pa_card_profile_info *get_active_profile(pa_context *context, char *card_name) {
    static card_profile_info info;
    ...
    info.active_profile = NULL;
    info.found = false;

    pa_operation *op = pa_context_get_card_info_by_name(context, card_name, get_active_profile_cb, &info);
    iterate(op);

    //fprintf(stderr, "[DEBUG, get_active_profile()] info.active_profile is %s\n", info.active_profile->description);
    return info.active_profile;
}
