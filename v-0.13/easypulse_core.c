 ...
 * @return Returns 0 on success, -1 on failure.
 */
int manager_move_output_playback(pulseaudio_manager *manager, uint32_t sink1_index, uint32_t sink2_index) {
    if (!manager || sink1_index >= manager->output_count || sink2_index >= manager->output_count) {
        fprintf(stderr, "Invalid arguments provided.\n");
        return -1;
    }

    _shared_data_1 shared_data = { manager, sink2_index };

    // Lock the main loop for thread safety
    pa_threaded_mainloop_lock(manager->mainloop);

    // Iterate over all sink inputs and move them to the new sink
    pa_operation *op = pa_context_get_sink_input_info_list(manager->context, manager_switch_default_output_cb_2, &shared_data);
