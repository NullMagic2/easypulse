/**
 * @file mute-channel-input-demo.c
 * @brief Program to toggle mute state of specified channels on a selected PulseAudio input device.
 *
 * This program uses the EasyPulse library to interface with PulseAudio. It lists all available
 * input devices and allows the user to select one. After a device is selected, the program displays
 ...
 *
 * Usage:
 * 1. A list of available input devices is displayed.
...
int main(void) {
    ...
    while(keep_running) {
        ...
        pulseaudio_device *selected_device = &self->inputs[device_index];
        ...
        for (int i = 0; i < selected_device->max_channels; i++) {
            bool mute_state = get_input_channel_mute_state(self->context, self->mainloop, selected_device->index, i);
        ...
        while (token != NULL) {
            ...
            if (channel >= 0 && channel < selected_device->max_channels) {
                ...
                bool new_mute_state = !get_input_channel_mute_state(self->context, self->mainloop, selected_device->index, channel);
                manager_set_input_mute_state(self, selected_device->index, channel, new_mute_state);
                new_mute_state = get_input_channel_mute_state(self->context, self->mainloop, selected_device->index, channel);
                printf("The new mute state is, %s\n\n", new_mute_state ? "Muted" : "Unmuted");
            } else {
