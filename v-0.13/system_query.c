...
static void get_output_streams_cb(pa_context *c, const pa_sink_input_info *i, int eol, void *userdata) {
    ...
    // Resize the inputs array to accommodate one more pa_sink_input_info
    input_list->inputs = realloc(input_list->inputs, (input_list->num_inputs + 1) * sizeof(pa_sink_input_info));
    ...
    // Copy the sink input info to the newly allocated spot
    memcpy(&input_list->inputs[input_list->num_inputs], i, sizeof(pa_sink_input_info));
...
pa_card_profile_info *get_active_profile(pa_context *context, uint32_t card_index) {
    ...
    pa_operation *op = pa_context_get_card_info_by_index(context, card_index, card_info_cb, &info);
    iterate(op);
