 ...
 * It safely handles the deallocation process, first freeing the array of output_stream_info
 * and then the output_stream_list structure itself.
 *
 * Usage of this function is essential after you're done using an output_stream_list
 * to avoid memory leaks. This function safely handles NULL pointers, so it can be
 * called with output_stream_list structures that may not have been fully initialized.
 *
...
void output_streams_cleanup(output_stream_list *list) {
    if (list != NULL) {
        // Free the array of output_stream_info structures
        if (list->inputs != NULL) {
            free(list->inputs);
        }
...
/**
 * @brief Callback function for the sink input information retrieval.
 *
 * This function is called by the PulseAudio context for each sink input
 * retrieved by pa_context_get_sink_input_info_list(). It stores each
 * sink input's information in a dynamically growing array within
 * output_stream_list
 *
 * @param c The PulseAudio context.
...
static void get_output_streams_cb(pa_context *c, const pa_sink_input_info *i, int eol, void *userdata) {
    ...
    // Populate the output_stream_info structure
    input_list->inputs[input_list->num_inputs].index = i->index;
    input_list->inputs[input_list->num_inputs].parent_index = i->sink;
    strncpy(input_list->inputs[input_list->num_inputs].name, pa_proplist_gets(i->proplist, "media.name"), sizeof(input_list->inputs[input_list->num_inputs].name) - 1);
 ...
 * of sink inputs from the PulseAudio server. It allocates and populates an
 * instance of output_stream_list with the results. The caller is responsible
 * for freeing the memory allocated for the pa_sink_input_info structures and
 * the output_stream_list itself.
 *
 * This function will block until the operation is completed or fails. The
 * sink input list will not be modified after the function returns.
 *
 * @param context A pointer to an initialized and connected pa_context object.
 * @return A pointer to a output_stream_list containing all retrieved sink inputs,
...
/**
 * @brief Callback function for receiving information about each source output (input stream).
 *
 * This function is called by the PulseAudio API for each source output (input stream)
 * queried via the pa_context_get_source_output_info_list() function. It appends each
 * source output's information to the list provided by the user.
 *
 * The function will resize the underlying storage array to accommodate new entries as they
 * are received. If realloc fails, it logs an error message and returns early.
 *
 * @param c The PulseAudio context.
 * @param o Information about a source output (input stream).
 * @param eol End of the list indicator. If non-zero, all source outputs have been received.
 * @param userdata User data passed to the function, cast to input_stream_list.
 */
static void get_input_streams_cb(pa_context *c, const pa_source_output_info *o, int eol, void *userdata) {
    ...
    // Resize the inputs array to accommodate one more pa_source_output_info
    input_list->inputs = realloc(input_list->inputs, (input_list->num_inputs + 1) * sizeof(pa_source_output_info));
    if (input_list->inputs == NULL) {
        // Handle allocation failure; ideally by signaling an error to the main program
        fprintf(stderr, "Failed to allocate memory for source input list\n");
        return;
    }

    // Copy the source input info to the newly allocated spot
    memcpy(&input_list->inputs[input_list->num_inputs], o, sizeof(pa_source_output_info));
    input_list->num_inputs++;
}

/**
 * @brief Retrieves a list of current source outputs from the PulseAudio server.
 *
 * This function initiates an asynchronous operation to retrieve the list
 * of source outputs from the PulseAudio server. It allocates and populates an
 * instance of output_stream_list with the results. The caller is responsible
 * for freeing the memory allocated for the pa_source_output_info structures and
 * the output_stream_list itself.
 *
 * This function will block until the operation is completed or fails. The
 * source input list will not be modified after the function returns.
 *
 * @param context A pointer to an initialized and connected pa_context object.
 * @return A pointer to a output_stream_list containing all retrieved source outputs,
 *         or NULL if an error occurs.
 */
input_stream_list *get_input_streams(pa_context *context) {
    input_stream_list *input_list = malloc(sizeof(output_stream_list));
    if (!input_list) {
        fprintf(stderr, "Failed to allocate memory for pa_source_input_list\n");
        return NULL;
    }
    ...
    // Initiate the operation to get the list of source inputs
    pa_operation *op = pa_context_get_source_output_info_list(context, get_input_streams_cb, input_list);
    if (!op) {
        fprintf(stderr, "Failed to start operation to get source input list\n");
        free(input_list);
        return NULL;
    }
